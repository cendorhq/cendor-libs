...
| [**guardrails**](packages/cendor-guardrails) | unsafe input / output | A deterministic gate at four stages (input / tool call / tool output / output) — block / redact / flag, offline, audit-evidenced. |
...
| [**acttrace**](packages/cendor-acttrace) | no audit trail | Pre-send guard for secrets & PII (block / redact) **and** a tamper-evident, offline-verifiable decision log with compliance evidence packs. |
...
n = tokens.count(messages, model="gpt-4o")   # exact by default — tiktoken is a required dep
...
- **Minimal core** — the only required runtime dependency is `tiktoken` (offline, so exact token/cost numbers ship out of the box); provider SDKs and OpenTelemetry are *optional extras*.
- **600+ tests, all offline** — mocked provider clients, golden token counts, and property-based
...
- **uv workspace**, seven packages built with `hatchling`, published to PyPI via GitHub Actions trusted
