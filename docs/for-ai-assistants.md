...
| SDK `guard` vs acttrace `guard` | `with guard(policy=Policy.strict(), audit=log): …` — the SDK's **scope form**; raw acttrace form: `itc = guard(policy)` then `add_interceptor(itc)` | `await guard({ policy, audit }, async () => …)` — the SDK's **scope form**; raw acttrace form: `const itc = guard(policy)` then `addInterceptor(itc)` | Two shapes, one enforcement. The SDK's `guard` is acttrace's policy enforcement in **scope form** — entering/exiting installs and removes the interceptor on core's seam. acttrace's raw `guard(policy, audit)` only **builds** the interceptor; it gates nothing until installed with `add_interceptor`/`addInterceptor`. Don't hand the scope form to `add_interceptor`, and don't expect the raw form to enforce on its own. |
