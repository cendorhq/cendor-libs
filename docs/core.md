...
| `register(model, rates)` | — | Register **per-token** rates for a model the snapshot doesn't know. Survives `refresh()`. A rate of `0` is honoured (you said so); a **negative** one raises `InvalidRateError` at the call that wrote it, since core 1.21.0 / `@cendor/core` 3.8.0. (TS: `prices.register`.) |
