...
| **guardrails** local embedder (G2) | ✅ Python | 🚧 planned | `embeddings.local_embedder()` (the `[embeddings]` extra, **model2vec** static embeddings, numpy-only, no torch) is the zero-config `embed` for the semantic checks. **Python only** — no maintained model2vec JS port; in TS pass a BYO `embed` (transformers.js / a hosted endpoint) |
