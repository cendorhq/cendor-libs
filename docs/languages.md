...
| **guardrails** `task_adherence` (A3) | ✅ Python | 🚧 planned | BYO-judge alignment check at the `tool_call` stage (does the proposed call match the user's intent?), via `judge.task_adherence` + `Context.instruction`. The `@cendor/guardrails` helper is ported; the **SDK-JS** auto-threading of the instruction is the deferred parity tail |
